<script lang="ts">
    ...
    let editor_cnt = `#!/usr/bin/env python3
...
`;
    for (const raw_data of raw) {
        ...
        if (raw_data.server_to_client === 0) {
            if (raw_data.blob && raw_data.blob[raw_data.blob.length - 1] == 10) {
...
<div id="raw-replay-editor" class="rounded-bottom">{editor_cnt}</div>